...
def num_tokens_from_messages(texts, model="gpt-4o"):
...
def gpt3Turbo_completion(chat_prompt="", system="You are an AI that can give the answer to anything", temp=0.7, model="gpt-4o", max_tokens=1000, remove_nl=True, conversation=None):
    ...
    while True:
        try:
            ...
            retry += 1
            if retry >= max_retry:
                raise Exception("GPT4 error: %s" % oops)
